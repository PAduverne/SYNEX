...
def main():
    ...
    parser.add_argument('--injection-file',
                        type=str,
                        default='./parameters_cbc.dat',
                        help='File with the injections parameters')

    args = parser.parse_args()

    # Reading the table containing the CBC population
    parameters_cbc = Table.read(args.injection_file,
                                format='ascii.commented_header')

    # Setting some parameters for the simulations.
    # We use four values for the mass ratio, four for the total mass and
    # four for the time before merger, at least during the production phase
    if args.test:
        print("For now, we are still using the test parameters")
        mass_ratio = [1.1]
        log_M = [4]
        n_iter, burn_in = 60, 20

    else:
        mass_ratio = [1., 3., 10., 100.]
        log_M = [4, 5, 6, 7]
        n_iter, burn_in = 4000, 1500

    # Number of silmualtions for each couple {mass_ratio, log_M}
    n_sim = args.n_sim
    ...
    # Initialisation of the index for the CBC parameters table
    index = 0
    ...
    for i, q in enumerate(mass_ratio):
        for j, M in enumerate(log_M):
            for k in range(n_sim):
                ...
                # create the repertory and sub-repertories
                # containing the files
                rep_name = 'q_{}_Mtot_{}/{}'.format(q, M, k)
                ...
                for cut in DeltatL_cut:
                    ...
                    # (n_iter, n_walkers, burn_in, n_temps) =
                    # (2000  ,    64    , 500    , 10)
                    RunTimekwargs = {
                        "print_info": True,
                        "n_walkers": 64, # 20, # 96,
                        "n_iter": n_iter,# 30, # 8000, # 15000, #
                        "burn_in": burn_in, #20, # 4000, # 5000, #
                    ...
                    # Create a json file with all the necessary
                    # parameters for SYNEX
                    json_files_dict = {}
